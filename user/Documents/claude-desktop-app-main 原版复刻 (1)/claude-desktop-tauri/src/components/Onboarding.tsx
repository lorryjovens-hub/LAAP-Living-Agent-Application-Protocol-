...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    import('../utils/tauriAPI').then(m => {
      m.tauriAPI.resizeWindow(600, 520);
    }).catch(() => {});
  }, []);
  ...
  const handleFinish = () => {
    ...
    import('../utils/tauriAPI').then(m => {
      m.tauriAPI.resizeWindow(1300, 780);
    }).catch(() => {});
    ...
    if (mode === 'selfhosted') {
      ...
      try { import('../utils/tauriAPI').then(m => m.tauriAPI.openExternal('https://clawparrot.com')).catch(() => {}); } catch {}
    }
    if (workspace) {
